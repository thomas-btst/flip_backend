...
class UserRepositoryTest(
    ...
    @Test
    fun `should save an user successfully and find it by id`() = runTest {
        val user = user()
    ...
    @Test
    fun `should count users correctly`() = runTest {
        val count = 1L
        for (i in 1..count) {
    ...
    @Test
    fun `should find an user by email`() = runTest {
        val user = user()
        userRepository.save(user)
    ...
    @Test
    fun `should update reset password key`() = runTest {
        val user = user()
        val key = "KEY"
        userRepository.save(user)
    ...
    fun `should change password successfully`() = runTest {
        val user = user(
            password = "password",
            resetPasswordKey = VerificationKey(
                "KEY",
                Instant.now().plusSeconds(properties.security.verificationKey.validityInSeconds),
            ),
            enabled = false,
    ...
    @Test
    fun `should not change password if key is invalid`() = runTest {
        val user = user(
            resetPasswordKey = VerificationKey(
                "KEY",
                Instant.now().plusSeconds(properties.security.verificationKey.validityInSeconds),
    ...
    @Test
    fun `should not change password if key is expired`() = runTest {
        val user = user(
            resetPasswordKey = VerificationKey(
                "KEY",
    ...
    @Test
    fun `should update activation key`() = runTest {
    ...
    @Test
    fun `should activate account successfully`() = runTest {
        val user = user(
            activationKey = VerificationKey(
                "KEY",
                Instant.now().plusSeconds(properties.security.verificationKey.validityInSeconds),
            ),
            enabled = false,
    ...
    @Test
    fun `should not activate account if key is invalid`() = runTest {
        val user = user(
            activationKey = VerificationKey(
                "KEY",
                Instant.now().plusSeconds(properties.security.verificationKey.validityInSeconds),
    ...
    @Test
    fun `should not activate account if key is expired`() = runTest {
        val user = user(
            activationKey = VerificationKey(
                "KEY",
                Instant.now().minusSeconds(1L),
    ...
    @Test
    fun `should update user profile correctly`() = runTest {
        val user = user(
            firstName = "Firstname",
            lastName = "Lastname",
            phone = "0789302874",
    ...
    @Test
    fun `should update user logo correctly`() = runTest {
